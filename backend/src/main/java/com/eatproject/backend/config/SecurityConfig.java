...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/main","/api/member/**","/api/restaurants/**").permitAll()                                       // / (첫페이지)는 로그인 없이 모두 허용합니다.
...
//        용어: 시큐리티: Authentication == UserDetails == principal (사용자계정을 담는 클래스들)
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(Arrays.asList("http://localhost:5173", "http://3.38.43.101:5173", "http://localhost:3000", "http://43.203.165.206:8080"));
